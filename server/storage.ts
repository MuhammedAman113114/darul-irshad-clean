...
// Import DatabaseStorage and JsonStorage
import { DatabaseStorage } from './database-storage';
import { JsonStorage } from './json-storage';
import { USE_JSON_STORAGE } from './db';
...
export interface IStorage {
  ...
  // Missed Sections (optional - for auto-detection)
  getMissedSections?(filters?: any): Promise<any[]>;
  createMissedSection?(section: any): Promise<any>;
  updateMissedSection?(id: number, section: any): Promise<any>;
  bulkUpsertTimetable?(entries: InsertTimetable[]): Promise<Timetable[]>;
  
  // Period Definitions (optional)
  getPeriodDefinitions?(): Promise<any[]>;
  createPeriodDefinition?(definition: any): Promise<any>;
  updatePeriodDefinition?(id: number, definition: any): Promise<any>;
  deletePeriodDefinition?(id: number): Promise<boolean>;
}
...
// Storage factory - uses database when available, otherwise JSON storage
function createStorage(): IStorage {
  if (USE_JSON_STORAGE) {
    console.log('[STORAGE] Using JsonStorage - data persisted to db.json file');
    return new JsonStorage('./db.json');
  }
  ...
  try {
    ...
    console.log('[STORAGE] Falling back to JsonStorage');
    return new JsonStorage('./db.json');
  }
}
