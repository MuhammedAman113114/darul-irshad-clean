...
export async function registerRoutes(app: Express) {
  ...
  app.post("/api/results", isAuthenticated, async (req, res) => {
    try {
      const resultData = {
        ...req.body,
        uploadedBy: req.session.user.name, // Use name (text) instead of id (integer)
        // uploadDate is auto-generated by database with defaultNow()
      };
      
      const result = await storage.createResult(resultData);
      console.log(`📊 Result uploaded: ${resultData.examType} for ${resultData.courseType} Year ${resultData.year}`);
